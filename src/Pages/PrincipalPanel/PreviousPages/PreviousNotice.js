...
const PreviousNotice = () => {
  ...
  const EditHandler = (id) => {
    fetch(`https://the-school-network-server.vercel.app/GetEditNotice/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setEditnotice(data);
        setOpen(true);
  ...
  return (
    <div>
      {user.email === "principal@gmail.com" ? (
        <h1 className="text-center mt-8 text-5xl font-bold mb-8">
          All Previous Notice
        </h1>
      ) : (
        <h1 className="text-center mt-8 text-5xl font-bold mb-8">
          Notice For All Teachers
        </h1>
      )}
      <div className="previous_notice_section">
        {previousNotice?.map((notice) => (
          <div className="previous_notice_div shadow-lg bg-gray-300">
            ...
            <h4 className="text-xl mt-4 font-bold text-center text-blue-900 pb-8">
              {notice?.title}
            </h4>
            <h4 className=" font-bold text-end mt-4 pb-4 text-grau-900 date_text">
            ...
            <p className="text-xl font-bold  pt-8 text-blue-900">
              {notice?.noticeFrom}
            </p>
